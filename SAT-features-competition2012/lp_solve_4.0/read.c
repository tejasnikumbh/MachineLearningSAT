...
static struct constraint_name
{
  char                    name[NAMELEN];
  int                     row;
  struct  constraint_name *next;
...
static void error(short verbose, char *string)
{
  if(Verbose >= verbose)
    report(NULL, verbose, "%s on line %d", string, *lineno);
}
 ...
 */
static int init_read(short verbose)
{
  int ok = FALSE;
...
{
  if(/* Lin_term_count > 1 */ HadConstraint && HadVar){ /* not a bound */
    /* if(Lin_term_count < 2) */
    if(rs == NULL)
...
int add_constraint_name(char *name)
{
  ...
  if(!First_constraint_name) { /* first time only */
    if (CALLOC(First_constraint_name, 1, struct constraint_name) == NULL) {
      error(CRITICAL, "Not enough memory");
      return(FALSE);
    }
...
{
  int    i, index, nn_ind, row_nr = -1, row_nr0;
  ...
  */
      ...
      while(cp != NULL) {
        if(lp != NULL) {
          ...
          row_nr = cp->row;
  	  if(row_nr <= row_nr0) {
            error(CRITICAL, "Error: rows are not in order");
  	    return(FALSE);
  	  }
          if(cp->value) {
  	    lp->mat[nn_ind].row_nr = row_nr;
  	    lp->mat[nn_ind].value = cp->value;
  	    if (negate[row_nr])
  	      lp->mat[nn_ind].value = -lp->mat[nn_ind].value;
  	    nn_ind++;
  	  }
        }
	tcp = cp;
...
lprec *yacc_read(short verbose, char *lp_name, int *_lineno, int (*parse) (void), void (*delete_allocated_memory) (void))
{
  lprec *lp = NULL;
  REAL *orig_upbo;
  int stat = -1;
  ...
  if((stat != 0) || (CALLOC(relat, Rows + 1, short) != NULL)) {
    if(stat == 0)
      lp = make_lpext(Rows, Columns, Non_zeros, my_max(1,Non_zeros), lp_name);
    if ((stat != 0) || (lp != NULL)) {
      if(lp != NULL)
        lp->verbose = (short)Verbose;
	...
	if(Rows) {
	  ...
	  set_constr_type_ex(lp, 1, Rows, relat + 1);
